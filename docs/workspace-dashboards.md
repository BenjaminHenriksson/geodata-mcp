...
Fresh databases run `db/init/07_workspace_dashboard_audit.sql` automatically.
For an existing database, apply `db/migrations/006_workspace_dashboard_audit.sql`
**before** starting the updated MCP/viewer images. The migration is transactional and
idempotent; it does not backfill actor identities or invent old MCP events.

Example for an operator in the intended deployment (not a test command):

```sh
docker compose exec -T postgres psql -v ON_ERROR_STOP=1 -U postgres -d geodata \
  < db/migrations/006_workspace_dashboard_audit.sql
```
...
Create a fresh PostgreSQL container from `db/Dockerfile`, mount this worktree's
`db/init` read-only, use a dedicated container/volume and unused loopback port.
Use database name `geodata` (the existing role bootstrap grants CREATE on that name).
...
For upgrade verification, initialize from the baseline `db/init`, apply migration
006 twice, then run the suite. Repeat with a fresh database using the new init
scripts. Browser verification uses a separately started viewer and MCP service on
