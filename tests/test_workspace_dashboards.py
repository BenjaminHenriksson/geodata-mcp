...
def test_audit_storage_failure_prevents_dispatch(principal, monkeypatch):
    ...
    dispatched = []
    @audit.tool(lambda ctx, workspace_id=None: workspace)
    def fake(ctx=None):
...
def test_completion_failure_retains_start_and_warns(principal, monkeypatch):
    ...
    @audit.tool(lambda ctx, workspace_id=None: principal[2])
    def fake(ctx=None):
        monkeypatch.setattr(db, "app_pool", lambda: (_ for _ in ()).throw(RuntimeError("unavailable")))
...
def test_explicit_workspace_calls_keep_audit_and_query_attribution(principal):
    """Two conversations sharing a key must not attribute selected calls to its default."""
    key, key_id, default = principal
    selected = [sessions.get_or_create_workspace(key_id, f'chat_{n}', activate=False) for n in range(2)]
    with ThreadPoolExecutor(max_workers=2) as pool:
        results = list(pool.map(lambda w: server.query('SELECT 42', ctx=context(key), workspace_id=w.id), selected))
    for result, workspace in zip(results, selected, strict=True):
        assert not result.get('error') and not result.get('audit_warning')
        row = records('''SELECT c.workspace_id, r.workspace_id, q.workspace_id, q.api_key_id::text
                         FROM app.mcp_calls c JOIN app.mcp_call_results r USING(call_id)
                         JOIN app.query_log q ON q.mcp_call_id = c.call_id WHERE c.call_id = %s''',
                      (result['audit_call_id'],))[0]
        assert row == (workspace.id, workspace.id, workspace.id, key_id)
    assert records('SELECT id::text FROM app.workspaces WHERE api_key_id = %s AND is_active', (key_id,))[0][0] == default.id
    assert audit.current_workspace() is None


def test_explicit_unowned_workspace_cannot_run_or_create_audit(principal):
    before = records('SELECT count(*) FROM app.mcp_calls WHERE api_key_id = %s', (principal[1],))[0][0]
    result = server.query('SELECT 42', ctx=context(principal[0]), workspace_id=str(uuid.uuid4()))
    assert result.get('error') and 'audit_call_id' not in result
    assert records('SELECT count(*) FROM app.mcp_calls WHERE api_key_id = %s', (principal[1],))[0][0] == before
