...
NODES = [
    ...
    ("eneo", (252, 266), "service", "Eneo", "Chat UI & tool orchestration",
     ...
     [("Deployment", "Frontend, backend and background worker run as separate containers."),
      ("Model loop", "Eneo sends conversation and tool-result context to Gemma through its configured model connection."),
    ...
    ("model", (956, 266), "model", "Gemma", "Chat & image analysis",
     ...
     [("Model", "Gemma 4 31B processes text and images through a model API."),
      ...
      ("Boundary", "Prompts and supplied images are sent to the external model API. Actual visual token allocation depends on the model service.")], "services/worker/connectors/gemma_change.py"),
...
FLOWS = {
    "query": ("Ask a geodata question", "From a question to a grounded answer.", [
        ...
        ("Plan with the chat model", "Eneo sends conversation context to Gemma. The model can request tools.", ["chat-model"]),
    ...
    "gemma": ("Detect changes · Gemma", "The default image-comparison path.", [
        ...
        ("Compare crop pairs", "Up to four calls run concurrently inside the job. Each sends two images to Gemma, using a separate model context from Eneo chat.", ["image-model"]),
